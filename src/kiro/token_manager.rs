...
impl MultiTokenManager {
    ...
    fn select_next_credential(
        ...
        self.select_next_credential_excluding(model, group, &HashSet::new(), true)
    }
    ...
    fn select_next_credential_excluding(
        ...
        match mode {
            "round-robin" => {
                let discovery_rank = available
                    .iter()
                    .map(|(_, support)| usize::from(*support != CachedModelSupport::Confirmed))
                    .min()?;
                let priority = available
                    .iter()
                    .filter(|(_, support)| {
                        usize::from(*support != CachedModelSupport::Confirmed) == discovery_rank
                    })
                    .map(|(entry, _)| entry.credentials.priority)
                    .min()?;
                let mut bucket: Vec<_> = available
                    .iter()
                    .filter(|(entry, support)| {
                        usize::from(*support != CachedModelSupport::Confirmed) == discovery_rank
                ...
                if advance_selection_state {
                    cursors.insert(key, cursor.saturating_add(1));
                }
    ...
    pub async fn acquire_context(
        ...
        self.acquire_context_with_exclusions(model, group, &HashSet::new(), true, true)
            .await
    ...
    pub async fn acquire_context_excluding(
        ...
        self.acquire_context_with_exclusions(
            model,
            group,
            excluded_ids,
    ...
    /// 获取 API 调用上下文，并返回本次实际使用的调度模式。
    ///
    /// Admin 模型发现需要复用同一套凭据选择和 Token 刷新规则，但不应因只读查询
    /// 更新 `current_id` 或推进 round-robin 游标。
    async fn acquire_context_impl(
        ...
        update_current: bool,
        advance_selection_state: bool,
    ) -> anyhow::Result<(CallContext, String)> {
        self.acquire_context_with_exclusions(
            model,
            group,
            &HashSet::new(),
    ...
    ///
    /// 凭据选择复用正常请求的账号池规则，但只读查询不会改变 `current_id` 或推进
    /// round-robin 游标。失效 Token 会在查询前刷新。返回实际命中的凭据 ID 和
    /// 调度模式，供管理前端明确展示数据来源。
    pub async fn get_available_models_for_current(
        &self,
    ) -> anyhow::Result<(u64, ListAvailableModelsResponse, String)> {
        let (context, selection_mode) = self.acquire_context_impl(None, None, false, false).await?;
        let id = context.id;
        let response = self.refresh_model_cache_for(id, true).await?;
        Ok((id, response, selection_mode))
    }
    ...
    pub fn set_load_balancing_config(
        ...
        let new_mode = mode.unwrap_or_else(|| previous_mode.clone());
        let new_request_retry = request_retry.unwrap_or(previous_request_retry);
        let new_max_retry_credentials =
            max_retry_credentials.unwrap_or(previous_max_retry_credentials);

        if new_mode == previous_mode
            && new_request_retry == previous_request_retry
            && new_max_retry_credentials == previous_max_retry_credentials
        {
            return Ok(());
        }
        ...
        *self.load_balancing_mode.lock() = new_mode.clone();
        self.request_retry
            .store(new_request_retry, Ordering::Relaxed);
        self.max_retry_credentials
            .store(new_max_retry_credentials, Ordering::Relaxed);

        if let Err(err) = self.persist_load_balancing_config(
            new_mode.clone(),
            new_request_retry,
            new_max_retry_credentials,
        ) {
            *self.load_balancing_mode.lock() = previous_mode.clone();
            self.request_retry
...
mod tests {
    ...
    #[test]
    fn concurrent_load_balancing_and_self_heal_updates_preserve_all_fields() {
        let (dir, config_path) = issue51_credentials_path("concurrent_runtime_config");
        let config = Config::default();
        std::fs::write(&config_path, serde_json::to_vec_pretty(&config).unwrap()).unwrap();
        let config = Config::load(&config_path).unwrap();
        let manager = Arc::new(MultiTokenManager::new(config, vec![], None, None, false).unwrap());
        ...
        let scheduling_manager = Arc::clone(&manager);
        let scheduling_barrier = Arc::clone(&barrier);
        let scheduling = std::thread::spawn(move || {
            scheduling_barrier.wait();
            scheduling_manager
        ...
        let self_heal_manager = Arc::clone(&manager);
        let self_heal_barrier = Arc::clone(&barrier);
        let self_heal = std::thread::spawn(move || {
            self_heal_barrier.wait();
            self_heal_manager
                .set_self_heal_config(Some(false), Some(false), Some(123), Some(7))
                .unwrap();
        });

        barrier.wait();
        scheduling.join().unwrap();
        self_heal.join().unwrap();
        ...
        assert!(!persisted.suspended_detection_enabled);
        assert!(!persisted.self_heal_enabled);
        assert_eq!(persisted.self_heal_min_interval_secs, 123);
        assert_eq!(persisted.self_heal_max_consecutive_rounds, 7);
    ...
    fn test_round_robin_excluding_high_priority_reaches_lower_priority() {
        ...
        assert_eq!(
            manager
                .select_next_credential_excluding(None, None, &excluded, true)
                .unwrap()
                .0,
            3
        );
    ...
    async fn balanced_read_only_selection_does_not_update_current_id() {
        ...
        let (context, selection_mode) = manager
            .acquire_context_impl(None, None, false, false)
            .await
            .unwrap();

        assert_eq!(selection_mode, "balanced");
    ...
    #[tokio::test]
    async fn round_robin_read_only_selection_does_not_advance_cursor() {
        let mut config = Config::default();
        config.load_balancing_mode = "round-robin".to_string();
        ...
        let (first_read, first_mode) = manager
            .acquire_context_impl(None, None, false, false)
            .await
            .unwrap();
        let (second_read, second_mode) = manager
            .acquire_context_impl(None, None, false, false)
            .await
            .unwrap();

        assert_eq!(first_mode, "round-robin");
        assert_eq!(second_mode, "round-robin");
        assert_eq!(first_read.id, 1);
        assert_eq!(second_read.id, 1);
        assert_eq!(manager.snapshot().current_id, 1);

        let first_request = manager.acquire_context(None, None).await.unwrap();
        let second_request = manager.acquire_context(None, None).await.unwrap();
        assert_eq!(first_request.id, 1);
        assert_eq!(second_request.id, 2);
    }
