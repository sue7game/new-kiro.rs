...
impl AdminService {
    ...
    pub fn get_load_balancing_mode(&self) -> LoadBalancingModeResponse {
        LoadBalancingModeResponse {
            ...
            request_retry: self.token_manager.get_request_retry(),
            max_retry_credentials: self.token_manager.get_max_retry_credentials(),
        }
    }
    ...
    pub fn set_load_balancing_mode(
        ...
        if let Some(value) = req.request_retry {
            if value > MAX_REQUEST_RETRY {
                return Err(AdminServiceError::InvalidCredential(format!(
                    "requestRetry 不能超过 {}，当前为 {}",
                    MAX_REQUEST_RETRY, value
                )));
            }
        }
        if let Some(value) = req.max_retry_credentials {
            if value > MAX_RETRY_CREDENTIALS {
                return Err(AdminServiceError::InvalidCredential(format!(
                    "maxRetryCredentials 不能超过 {}，当前为 {}",
                    MAX_RETRY_CREDENTIALS, value
                )));
            }
        }

        if req.mode.is_none() && req.request_retry.is_none() && req.max_retry_credentials.is_none()
        {
            return Err(AdminServiceError::InvalidCredential(
                "至少需要提供一个负载均衡配置字段".to_string(),
            ));
        }

        self.token_manager
            .set_load_balancing_config(req.mode, req.request_retry, req.max_retry_credentials)
