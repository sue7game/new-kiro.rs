...
impl ClientKeyManager {
    ...
    ///
    /// 积分达到上限时返回 `None`；需要区分原因时用 [`Self::verify_and_touch_ex`]。
    #[allow(dead_code)]
    ...
    pub fn verify_and_touch_ex(&self, presented: &str) -> KeyAuth {
        let mut inner = self.inner.write();
        let Some(id) = find_active_key_id(&inner, presented) else {
...
mod tests {
    ...
    fn sync_system_key_uses_id_zero() {
        ...
        assert_eq!(mgr.verify("custom-api-key"), Some(0));
        assert_eq!(mgr.list()[0].total_calls, 0);
    ...
    #[test]
    fn sync_system_key_migrates_legacy_non_system_entry_to_id_zero() {
        let mgr = ClientKeyManager::new();
