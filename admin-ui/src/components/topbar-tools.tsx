...
export function TopbarTools({ compact = false }: TopbarToolsProps) {
  ...
  const loadBalancingMode = lbData?.mode ?? 'priority'
  const nextMode = nextLoadBalancingMode(loadBalancingMode)
  ...
  const toggles: ToggleSpec[] = [
    {
      key: 'lb',
      on: loadBalancingMode !== 'priority',
      busy: lbLoading || lbSaving,
      label: lbLoading ? '加载中…' : loadBalancingLabels[loadBalancingMode],
      menuLabel: `切换到${loadBalancingLabels[nextMode]}`,
      title: `调度模式：${loadBalancingLabels[loadBalancingMode]}；点击切换到${loadBalancingLabels[nextMode]}`,
      icon: <Activity className="h-3.5 w-3.5" />,
      onToggle: () =>
        setLb(nextMode, {
          onSuccess: () => toast.success(`已切换到${loadBalancingLabels[nextMode]}模式`),
          onError,
        }),
    },
    {
